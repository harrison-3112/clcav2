...
function renderMesR001Rows(rows = mesR001Rows) {
    ...
    displayRows.forEach((row) => {
        ...
        const cellHtml = {
            ...
            DefectCode: `<td class="p-3 ${resultClass} whitespace-nowrap" title="${quickLogEscape(getMesR001Cell(row, 'DefectCode'))}">${quickLogEscape(getMesR001Cell(row, 'DefectCode'))}</td>`,
...
async function searchMesDashboard() {
    ...
    try {
        ...
        setStatus('generating', 'Querying MES Daily API...');
        showProgress();
        mesR001Rows = [];
        mesR001SelectedIndex = -1;
        renderMesR001Rows([]);
        ...
        renderMesR001Rows(mesR001Rows);
        completeProgress();
        setStatus('success', 'MES Daily data loaded');
...
// Demo zip download logic removed
