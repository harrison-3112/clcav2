...
module.exports = function createMesDailyRoutes(context = {}) {
  ...
  const { buildDashboardData } = require('../modules/mesdaily/dashboardAggregator');
  ...
  router.post('/api/mesdaily/dashboard', async (req, res) => {
    try {
      ...
      const result = await buildDashboardData({
        ...
        timeto: body.timeto,
        selectedStations: Array.isArray(body.selected_stations)
          ? body.selected_stations.map((s) => String(s || '').trim()).filter(Boolean)
          : [],
        thresholds: body.thresholds || {},
        previousKpis: body.previousKpis || null,
      });
      return res.json(result);
    } catch (err) {
      console.error('[MES DAILY DASHBOARD ERROR]', err && err.stack ? err.stack : err);
